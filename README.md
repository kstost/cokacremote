...
Docker Compose mounts `${CONFIG_PATH:-./config}` at `/config` so the authenticated dashboard can save policy changes. Copy `config/safety-policy.example.json` to `config/safety-policy.json`, set `MCP_SAFETY_MODE=safe` and `MCP_SAFETY_POLICY_FILE=/config/safety-policy.json`, then restart the service once to load the configured file path. After that, `/dashboard` can validate, save, and reload the policy without restarting the server. Each successful save/reload is appended to `<policy-file>.history.jsonl` with a timestamp, revision ID, SHA-256 digest, and validated policy snapshot. The dashboard shows this history and can atomically roll back to any retained revision; rollback itself creates a new audit revision. `npm run doctor` validates the configured policy file and fails if the JSON, regex, version, or decision values are invalid.
