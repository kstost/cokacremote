...
(function(){
  ...
  function matchesTask(t){var q=search.trim().toLowerCase();if(q){var hay=(t.title+' '+(t.cwd||'')+' '+(t.commands||[]).map(function(c){return c.command;}).join(' ')).toLowerCase();if(!hay.includes(q))return false;}var state=taskState(t);if(filter==='active')return state.code==='active';if(filter==='warning')return state.code==='warning';if(filter==='failed')return state.code==='failed';if(filter==='completed')return state.code==='completed';return true;}
