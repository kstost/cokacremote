...
document.getElementById('logoutBtn').onclick=async()=>{await fetch('./logout',{method:'POST'});location.href='./login'};document.getElementById('policyBtn').onclick=()=>{selected=null;loadTasks();loadPolicy()};loadApprovals();loadTasks();setInterval(()=>{loadApprovals();loadTasks();if(selected)loadDetail()},3000);
</script></body></html>`;
