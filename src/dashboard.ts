...
async function loadPolicy(){try{const d=await api('./api/policy');const text=JSON.stringify(d.policy??{version:1},null,2);document.getElementById('detail').innerHTML='<h2>Safety policy</h2><div class="muted">Mode: '+esc(d.mode)+' · '+esc(d.file||'no policy file configured')+'</div><div id="policyNotice"></div><textarea id="policyEditor" class="editor">'+esc(text)+'</textarea><div class="toolbar" style="margin-top:10px"><button id="validatePolicy" class="btn">Validate</button><button id="savePolicy" class="btn">Save & reload</button><button id="reloadPolicy" class="btn">Reload from disk</button></div>';const note=(m,c)=>document.getElementById('policyNotice').innerHTML='<div class="notice '+c+'">'+esc(m)+'</div>';document.getElementById('validatePolicy').onclick=async()=>{try{const policy=JSON.parse(document.getElementById('policyEditor').value);await api('./api/policy/validate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(policy)});note('Policy is valid.','ok')}catch(e){note(String(e),'error')}};document.getElementById('savePolicy').onclick=async()=>{try{const policy=JSON.parse(document.getElementById('policyEditor').value);await api('./api/policy',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(policy)});note('Saved and reloaded. New requests use this policy immediately.','ok')}catch(e){note(String(e),'error')}};document.getElementById('reloadPolicy').onclick=async()=>{try{await api('./api/policy/reload',{method:'POST'});await loadPolicy()}catch(e){note(String(e),'error')}}}catch(e){document.getElementById('detail').innerHTML='<div class="empty">'+esc(String(e))+'</div>'}}
