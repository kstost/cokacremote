...
</style></head><body><header><div><div class="brand">cokacremote</div><div class="muted">Development task timeline</div></div><span id="refresh" class="muted">refreshing...</span></header><div class="layout"><aside><div id="approvals"></div><div id="tasks"></div></aside><main id="detail"><div class="empty">Select a task to inspect its timeline.</div></main></div>
<script>
...
loadApprovals();loadTasks();setInterval(()=>{loadApprovals();loadTasks();if(selected)loadDetail()},3000);
</script></body></html>`;

export function registerDashboard(app: Express, config: AppConfig, journal: TaskJournal, safetyPolicy: SafetyPolicy, authenticate: Middleware): void {
